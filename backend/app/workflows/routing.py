"""Routage local : seuls les cas non documentaires explicites évitent le RAG."""
import re
import unicodedata
...
def select_route(message: str, mode: str = 'auto') -> str:
    ...
    if text in {'hello', 'hi', 'hey', 'bonjour', 'salut', 'coucou', 'bonsoir', 'can you help', 'peux-tu m’aider'}:
        return 'greeting'
    if re.fullmatch(r'(?:list (?:the )?(?:indexed )?documents|(?:liste|lister) (?:les |des )?(?:documents|fichiers)(?: indexes)?|quels (?:documents|fichiers) sont indexes)', text):
        return 'document_list'
    # Une date ou une opération citée dans une question métier n'est pas une demande de calcul.
    ...
    if re.fullmatch(r'[\d\s.,()+*/%^\-]+', expression) and re.search(r'\d', expression) and re.search(r'[+*/%^\-]', expression):
        return 'calculation'
    # Transformer un texte explicitement fourni peut se faire sans recherche documentaire.
