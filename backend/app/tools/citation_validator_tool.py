"""Validate citation labels generated by the RAG workflow."""
...
class CitationValidatorTool:
    """Check citation presence and range, with an optional lexical-support signal."""
    ...
    def run(self, answer: str, documents: list[SearchResult]) -> ToolResult:
        ...
        # A no-documents result has nothing to cite; validation is
        # successful but explicitly marked as skipped in metadata.
        ...
        if not documents:
            output = "Validation skipped: no RAG documents available."
        elif missing:
            output = "The RAG answer contains no [n] citations."
        elif invalid_labels:
            output = f"Out-of-range citations detected: {invalid_labels}."
        elif unsupported_labels:
            output = (
                f"Structurally valid citations: {cited_labels}. "
                f"Weak lexical support: {unsupported_labels}."
            )
        else:
            output = f"Valid citations with lexical support: {cited_labels}."
    ...
    def _support_by_label(
        ...
        """Compare terms in the citing sentence with terms in the referenced document."""
        sentences = [
            segment.strip()
    ...
    def _terms(self, text: str) -> set[str]:
        """Tokenize text for a lightweight, auditable citation-support signal."""
