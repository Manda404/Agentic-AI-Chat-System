...
- Envoie `state.user_message` tel quel à Elasticsearch via `SearchService.search()`.
- Requête `multi_match` sur les champs `title^2`, `snippet`, `category` (le titre
  pèse 2x plus que le reste).
...
- Si Elasticsearch est indisponible ou l'index n'existe pas, `SearchService.search()`
  lève une exception — mais le nœud du graphe est configuré avec
  `swallow_errors=True`, donc le workflow continue avec `search_results = []`
...
- Conçu pour fusionner les résultats Elasticsearch avec une recherche vectorielle
  optionnelle (`VectorStorePort`).
- Aujourd'hui, aucun store vectoriel n'est branché : `NullVectorStore` retourne
  toujours `[]`. En pratique, cet agent ne fait donc que **dédupliquer et trier**
  les résultats déjà produits par `SearchAgent` (voir *Erreur #2*).
...
1. **Score lexical** (toujours calculé) : score Elasticsearch brut + bonus
   `0.25` par mot de la question retrouvé dans le titre/snippet.
2. **Score sémantique** (si `SEMANTIC_RERANKER_ENABLED=true`) : similarité
   ...
   calculés via `HuggingFaceEmbeddingService` (modèle `MODEL_EMBEDDING`,
   défaut `BAAI/bge-small-en-v1.5`), en un seul appel batch.
...
| `ELASTICSEARCH_INDEX` | Index interrogé par `SearchService` | `starter_documents` |
...
### 1. `MAX_RAG_DOCUMENTS` n'a aucun effet sur le nombre de documents *récupérés*
**Fichier :** [`search_service.py:124`](../services/search_service.py) (`size=5` codé en dur)

`SearchService.search()` fixe `size=5` sans lire `settings.max_rag_documents`.
Résultat : même si vous configurez `MAX_RAG_DOCUMENTS=20`, Elasticsearch ne
renverra jamais plus de 5 candidats — le reranker n'a donc jamais plus de 5
documents à trier, quel que soit le réglage. Le setting ne contrôle en
pratique que la troncature *après* reranking, pas la profondeur de recherche.

### 2. `HybridRetrieverAgent.limit=8` est inopérant dans la configuration actuelle
**Fichier :** [`hybrid_retriever_agent.py:25`](hybrid_retriever_agent.py)

Conséquence directe de l'erreur #1 + `NullVectorStore` toujours vide : la
liste fusionnée ne dépasse jamais 5 éléments, donc `[: self.limit]` (limite
à 8) ne s'active jamais. Ce n'est pas un bug qui casse quelque chose
aujourd'hui, mais un paramètre qui donne une fausse impression de contrôle.
...
Le score final additionne le score lexical (issu du score Elasticsearch BM25,
dont l'échelle dépend du corpus et peut dépasser largement 2) et le score
sémantique (cosinus ∈ [-1, 1], multiplié par 2, donc borné à [-2, 2]). Sur un
corpus où les scores Elasticsearch sont élevés, la contribution sémantique
peut devenir négligeable dans le classement final — l'ajout de l'embedding
n'aurait alors presque aucun effet observable. Cette pondération mériterait
d'être testée/calibrée sur des données réelles plutôt que fixée arbitrairement.
...
- #1/#2 : lire `settings.max_rag_documents` (ou une nouvelle variable dédiée,
  ex. `SEARCH_TOP_K`) dans `SearchService.search()` au lieu de `size=5` fixe.
...
- #6 : mesurer la distribution réelle des scores Elasticsearch sur le corpus
  du projet avant de choisir un poids, ou normaliser les deux scores sur une
  échelle commune (ex. min-max) avant de les combiner.
